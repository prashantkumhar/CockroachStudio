...
const FALL_END = 0.4; // fraction: gravity drop
...
export async function renderMemeVideo(config: RenderConfig): Promise<Blob> {
  ...
  await new Promise<void>((resolve) => {
    ...
    const id = setInterval(() => {
      ...
      const imageZoom =
        frame < PRE_FRAMES ? 1.25 - 0.25 * easeOutBack(frame / PRE_FRAMES) : 1;
      ...
      for (let si = 0; si < numSlots; si++) {
        ...
        drawTextSlot(ctx, resolveTextSlot(slot, textStyles?.[si]), text, W, H);
        ctx.restore();
