...
function boxBlurH(
  ...
  for (let y = 0; y < H; y++) {
    for (let c = 0; c < 3; c++) {
      let sum = 0;
      for (let x = -r; x <= r; x++)
        sum += src[(y * W + Math.max(x, 0)) * 4 + c];
      const inv = 1 / (2 * r + 1);
      for (let x = 0; x < W; x++) {
        dst[(y * W + x) * 4 + c] = sum * inv;
        const lo = Math.max(x - r, 0),
          hi = Math.min(x + r + 1, W - 1);
...
function boxBlurV(
  ...
  for (let x = 0; x < W; x++) {
    for (let c = 0; c < 3; c++) {
      let sum = 0;
      for (let y = -r; y <= r; y++)
        sum += src[(Math.max(y, 0) * W + x) * 4 + c];
      const inv = 1 / (2 * r + 1);
      for (let y = 0; y < H; y++) {
        dst[(y * W + x) * 4 + c] = sum * inv;
        const lo = Math.max(y - r, 0),
          hi = Math.min(y + r + 1, H - 1);
...
export function cartoonizePixels(
  src: Uint8ClampedArray,
  W: number,
  H: number,
): Uint8ClampedArray {
  ...
  for (let i = 0; i < smooth.length; i += 4) {
    ...
    const [r, g, b] = hslToRgb(
      h,
      Math.min(1, s * SAT_BOOST),
      Math.round(l * COLOR_LEVELS) / COLOR_LEVELS,
    );
    cel[i] = r;
    cel[i + 1] = g;
    cel[i + 2] = b;
